...
def signup():
    ...
    try:
        ...
        with conn.cursor() as cursor:
            if role == 'owner':
                query = """
                    INSERT INTO seller (username, email, password, seller_city, restaurant_name, restaurant_address)
                    VALUES (%s, %s, %s, %s, %s, %s)
                """
                values = (
                    data.get('username',''),
                ...
                query = """
                    INSERT INTO user (username, email, password, user_city)
                    VALUES (%s, %s, %s, %s)
                """
                values = (
                    data.get('username',''),
...
def login():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT * FROM seller WHERE email = %s AND password = %s",
                (data.get('email'), data.get('password'))
            )
...
def decline_request(id):
    cursor.execute("""
        UPDATE requests SET status='declined' WHERE id=%s
    """, (id,))
